...
void main() {
  ...
  group('Transforming Palettes', () {
    test('Inverted', () {
      final colorPalette = ColorPalette(List<ColorModel>.from(_testColors));
  ...
  group('List Utilities', () {
    final colors = _testColors.sublist(3);
    ...
    test('expand', () {
      final colorSpaces = ColorSpace.values;
      Iterable<ColorModel> expander(ColorModel color) =>
          colorSpaces.map<ColorModel>((colorSpace) => colorSpace.from(color));
    ...
    test('reduce', () {
      expect(colors.reduce((a, b) => a.interpolate(b, 0.5)),
          equals(colorPalette.reduce((a, b) => a.interpolate(b, 0.5))));
    });

    test('fold', () {
      expect(
        colors.fold<RgbColor>(
            colors.first as RgbColor, (a, b) => a.interpolate(b, 0.5)),
        equals(colorPalette.fold<RgbColor>(
            colorPalette.first as RgbColor, (a, b) => a.interpolate(b, 0.5))),
      );
    });
    ...
    test('sort', () {
      colorPalette.sort((a, b) => a.hue.compareTo(b.hue));
      expect(colors, equals(colorPalette));
    });
    ...
    test('indexWhere', () {
      expect(colors.indexWhere((color) => color.hex == '#ff0000'),
          equals(colorPalette.indexWhere((color) => color.hex == '#ff0000')));
    });
    ...
    test('+', () {
      expect(colors + colorPalette, equals(colorPalette + colors));
    });
